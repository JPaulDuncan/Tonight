...
> trees and rocks, build walls, floors, ramps and cones on the grid — with a
> preview ghost, structural collapse, and the build-HP ramp that makes a fresh
> wall killable — fight bots that shoot back, with shields, damage numbers and
> elimination, and outrun a storm that closes the island from dusk to sunrise.
> It is not yet a match: no other players and no networking.
>
> 313 simulation tests run in under two seconds, and CI drives the built game in a
...
| `1` … `6` | pick a weapon, out of build mode |
| Left-click | place a piece, fire, or swing the pickaxe |
| `R` | reload · hold `G` and sweep: edit a wall · `T` revert |
| `F H J K` | shield potions, bandage, med kit |
| `P` | pause the storm |
...
That boundary is what makes 313 tests run headless in under two seconds, and it is
